...
class _SelectiveScanPyTorchFallback:
    """Pure-PyTorch fallback when CUDA selective_scan extensions are not compiled."""

    @staticmethod
    def fwd(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, *extra):
        dtype_in = u.dtype
        u = u.float()
        delta = delta.float()
        ...
        if B_float.dim() == 3:
            deltaB_u = torch.einsum('bdl,bnl,bdl->bdln', delta, B_float, u)
        elif B_float.dim() == 4:
            B_exp = B_float.repeat(1, dim // B_float.shape[1], 1, 1)
            deltaB_u = torch.einsum('bdl,bdnl,bdl->bdln', delta, B_exp, u)
        else:
            deltaB_u = torch.einsum('bdl,dn,bdl->bdln', delta, B_float, u)

        if C_float.dim() == 4:
            C_float = C_float.repeat(1, dim // C_float.shape[1], 1, 1)

        for i in range(u.shape[2]):
            x = deltaA[:, :, i] * x + deltaB_u[:, :, i]
            if C_float.dim() == 3:
                y = torch.einsum('bdn,bn->bd', x, C_float[:, :, i])
            elif C_float.dim() == 4:
                y = torch.einsum('bdn,bdn->bd', x, C_float[:, :, :, i])
            else:
                y = torch.einsum('bdn,dn->bd', x, C_float)
            ys.append(y)
    ...
    @staticmethod
    def bwd(*args):
        raise NotImplementedError("Backward not supported without CUDA selective_scan extensions")

_selective_scan_fallback = _SelectiveScanPyTorchFallback()

try:
    import selective_scan_cuda_oflex
except Exception as e:
    selective_scan_cuda_oflex = _selective_scan_fallback

try:
    import selective_scan_cuda_core
except Exception as e:
    selective_scan_cuda_core = _selective_scan_fallback

try:
    import selective_scan_cuda
except Exception as e:
    selective_scan_cuda = _selective_scan_fallback
